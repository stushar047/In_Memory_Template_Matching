...
from src.allf import resize_image, Data2Current, mem_pcc, mem_pcc2, soft_pcc, soft_pcc_approximate, scale_values
...
class Template_matching:
    ...
    img_direc: Directory containing the images
    ...
    def __init__(self, model, img_direc="PeanutBottle/", temp_img="template/template2.jpg"):
        ...
        self.Result_dict = {
            "Img": [], "mcorr": [], "mbox": [], "scorr": [], "sbox": [], 
            "scorr2": [], "sbox2": [], "mse": [], "mse2": [], "E": []
        }
    ...
    def dataset_creation(self):
        ...
        Attributes:
        img_list: the list of names of the images in the dataset
        full_img_list: the list of the images in the dataset
        ...
        self.template = resize_image(cv2.imread(self.temp_img, 0), scale_factor)
    ...
    def pcc_computation(self, full_image, row_stride=20, col_stride=20, nl=0):
        ...
        Input Parameters:
        full_image: The image with which PCC needs to be computed with template
        ...
        """
        iter_row = (full_image.shape[0] - self.temp_row) // row_stride
        iter_col = (full_image.shape[1] - self.temp_col) // col_stride
        ...
        for ir in range(iter_row + 1):
            for ic in range(iter_col + 1):
                Data_sample = full_image[
                    ir * row_stride:ir * row_stride + self.temp_row,
                    ic * col_stride:ic * col_stride + self.temp_col
                ].reshape(-1,)
    ...
    def calculate_metrics(self, Corr_template, Corr_template_soft, Corr_template_soft2, I, SMA, E, nl=0, row_stride=20, col_stride=20):
        ...
        if nl == 0:
            ...
            my0, my1, mx0, mx1 = idx_2d[0] * row_stride, idx_2d[0] * row_stride + self.temp_row, idx_2d[1] * col_stride, idx_2d[1] * col_stride + self.temp_col
            ...
            sy0, sy1, sx0, sx1 = idx_2d[0] * row_stride, idx_2d[0] * row_stride + self.temp_row, idx_2d[1] * col_stride, idx_2d[1] * col_stride + self.temp_col
            ...
            say0, say1, sax0, sax1 = idx_2d[0] * row_stride, idx_2d[0] * row_stride + self.temp_row, idx_2d[1] * col_stride, idx_2d[1] * col_stride + self.temp_col
